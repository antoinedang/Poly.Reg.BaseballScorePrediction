...
def updateHighScore(accuracy, mse, name, parameters, pred, true, n_comp):
    try:
        ...
        lowestMSE = float(data[2][23:].strip()[:-1])
    except Exception:
        data = [""]
    ...
    try:
        ...
        ## 18 -> 20 
        highestAccuracy = float(data[3][12:20])
    except:
        data = [""]
        highestAccuracy = 0.0
    ...
    output = ""
    if (float(mse) < float(lowestMSE)):
        print("New mse: " + str(mse) + "      Old mse: " + str(lowestMSE))
    ...
    try:
        data = open("bestofBoth.txt", 'r').readlines()
    ...
    try:
        ...
        highestAccuracy = 0.0
    output = ""
    if (float(mse) <= float(lowestMSE)) and ((accuracy >= highestAccuracy)):
        print("New mse: " + str(mse) + "      Old mse: " + str(lowestMSE))
        output += "Best of Both:\n"
        output += "\t" + name + "\n"
        output += "\t\t" + "Mean Squared Error: " + str(mse) + "\n"
        ...
        bestMSE = open("bestofBoth.txt", 'w')
        bestMSE.write(output)
        bestMSE.close()
#function that prints the score of a model's prediction on a stat set
def score(pred, true, name, parameters, reverse, startTime, n_comp, alphaA):
    ...
    print("===== " + name + " =====" + str(startTime) + " =====")
    print('Alpha: ' + str(alphaA))
...
def start():
    ...
    if (use_scaling):
        ...
        ## TESTS
        print("Mean training x scaled: " + str(training_x.mean()))
        print("Mean test x scaled: " + str(test_x.mean()))
    ...
    for component in hyperparameters[0][4:6]:
        for alpha in [x * .00001 for x in range(2000//25, 20000, 2000//25)]:
            ...
            model = skm.Ridge(alpha=alpha, max_iter=5000, tol=0.00001, solver='auto')
            model.fit(training_x, training_y)
            predictedScores = model.predict(test_x)
            score(predictedScores, test_y, "Ridge", model.get_params(), False, startTime, component, alpha)

            """ startTime = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
            model = skm.LinearRegression(fit_intercept=True, normalize='deprecated', copy_X=True, n_jobs=-1, positive=False)
            model.fit(training_x, training_y)
            predictedScores = model.predict(test_x)
            score(predictedScores, test_y, "Linear", model.get_params(), False, startTime, component, alpha) """
    ...
    print("start() finished running. BBModel Done.")
